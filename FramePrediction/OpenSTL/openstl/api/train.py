...
class BaseExperiment(object):
    ...
    def __init__(self, args, wandb_config, dataloaders=None):
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            if epoch % self.args.log_step == 0:
                ...
                with torch.no_grad():
                    vali_loss = self.vali()
                wandb.log({'epoch': epoch + 1, 'step':len(self.train_loader), 'current_lr':cur_lr, 'train_loss': loss_mean.avg, 'validation_loss': vali_loss})
    ...
    def vali(self):
        ...
        wandb.log({"validation_mse": results["mse"].mean(), "validation_mae": results["mae"].mean()})
    ...
    def test(self):
        ...
        if self._rank == 0:
            #print({"test_mse": eval_res["mse"], "test_mae": eval_res["mae"]})
            wandb.log({"test_mse": eval_res["mse"], "test_mae": eval_res["mae"]})
            print_log(eval_log)
    ...
    def inference(self):
        ...
        check_dir(folder_path)
        torch.save(torch.from_numpy(results["preds"]), osp.join(folder_path, "preds.pt"))
        # np.save(osp.join(folder_path, np_data + '.npy'), results[np_data])
